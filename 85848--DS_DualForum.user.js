...
if(document.location.href.indexOf("forum.php") != -1) {
    ...
    // Handle Thread
    if (document.location.href.match("screen=view_thread&thread") && document.location.href.indexOf('answer=true') == -1) {
		...
		if(!table.getElementsByTagName('td')[1]) {
		   // Some error: http://forum.die-staemme.de/showpost.php?p=2871865&postcount=34
		   var lastpage = true
		} 
		else {
		...
		if(alist.length == 0) {
		  ...
		  var lastPage = parseInt(alist[alist.length-1].href.match(/page=([0-9]+)/)[1])+1;
        ...
        if(lastpage) {    // Mark Thread read on last page
			...
			if(lastpost.getElementsByTagName('span')[1].innerHTML.match(/id=([0-9]+)/))		
              var pid = lastpost.getElementsByTagName('span')[1].innerHTML.match(/id=([0-9]+)/)[1];
			else // DS Duke
              var pid = lastpost.getElementsByTagName('span')[2].innerHTML.match(/id=([0-9]+)/)[1];
        ...
        for (i = 0; i < posts.length; i++ ) {
			...
			if(!matches)
            {
				// Duke and Forum Assistant added a span
				var matches = posts[i].getElementsByTagName('span')[2].innerHTML.match(/quote_id=([0-9]+)/)				
			}				
            ...
            window.setInterval(function() {
                ...
                for (i = 0; i < redPosts.length; i++ ) {
                    ...
                    { // Mark Red	
                        ...
                        {
				            // Duke and Forum Assistant added a span
				            var matches = redPosts[i].getElementsByTagName('span')[2].innerHTML.match(/quote_id=([0-9]+)/)				
				        }			
                        var post_id = matches[1];	  
